...
function drawStars() {
    ...
    for (let i = 0; i < stars.length; i++) {
        ...
        showerCtx.beginPath();
        showerCtx.arc(star.x, star.y, star.size, 0, Math.PI * 2);
        showerCtx.fill();
    }
    showerCtx.restore();
}
...
function drawShower() {
    ...
    showerParticles.forEach(p => {
        ...
        if (!p.type || p.type === 'circle') {
            ...
            // 擬似的なグロー効果：より大きく透明な円を薄く重ねる
            showerCtx.fillStyle = `hsla(${p.hue}, 85%, 72%, ${p.alpha * 0.25})`;
            showerCtx.beginPath();
            showerCtx.arc(p.x, p.y, p.size * 1.8, 0, Math.PI * 2);
            showerCtx.fill();
        } else {
            ...
            if (p.type === 'sparkle') {
                ...
                // スパークルの擬似グロー
                showerCtx.fillStyle = `hsla(${currentHue}, 90%, ${currentLightness}%, ${p.alpha * 0.25})`;
                showerCtx.beginPath();
                showerCtx.arc(0, 0, currentSize * 3.0, 0, Math.PI * 2);
                showerCtx.fill();
...
function drawBubbles() {
    ...
    bubbles.forEach(b => {
        ...
        if (b.type === 'silver') {
            ...
            showerCtx.beginPath();
            showerCtx.arc(0, 0, drawRadius * 2.0, 0, Math.PI * 2);
            showerCtx.fill();
            
            // メインの銀色の球体
            const bodyGrad = showerCtx.createRadialGradient(
                -drawRadius * 0.2, -drawRadius * 0.2, drawRadius * 0.08,
                0, 0, drawRadius
            );
            ...
            // 銀色の輪郭
            showerCtx.strokeStyle = 'rgba(255, 255, 255, 0.5)';
            ...
            showerCtx.fillStyle = glowGrad;
            showerCtx.beginPath();
            showerCtx.arc(0, 0, drawRadius * 1.8, 0, Math.PI * 2);
            showerCtx.fill();
            ...
            showerCtx.beginPath();
            showerCtx.arc(0, 0, drawRadius - 0.8, 0, Math.PI * 2);
            showerCtx.stroke();
...
function playMeteorBigExplosionSound(originX) {
    ...
    try {
        ...
        for (let i = 0; i < starCount; i++) {
            ...
            // B. 非調和倍音 2.76倍 (ウィンドチャイムの金属特有 of 非整数の響き)
...
function playCarbonatedBubbleSound(originX) {
    ...
    try {
        ...
        const durationSeconds = 6.5; // 持続時間 6.5秒
        ...
        for (let i = 0; i < bufferSize; i++) {
            ...
            leftData[i] = (Math.random() * 2 - 1) * amp;
            rightData[i] = (Math.random() * 2 - 1) * amp;
        }

        // 2. パチパチ音（400個）を一括加算
        const bubbleCount = 400;
        const clickDuration = durationSeconds - 0.2; // 6.3秒間発生
        ...
        for (let i = 0; i < bubbleCount; i++) {
            ...
            // 発生位置からパンを設定し、LeftとRightのゲインを計算
            const clickPan = Math.max(-1.0, Math.min(1.0, basePan + (Math.random() - 0.5) * 1.0));
