...
const newBlock = `function drawRealAuroraCurtain() {
    ...
    for (let x = 0; x < showerCanvas.width; x += step) {
        ...
        // 全体の発光量をさらに抑え（不透明度基本値を0.14から0.038へ）、背後が完全に透けるようにします
        // 時間経過による全体のゆっくりとした明滅・揺らぎ ＆ ひだの強弱を掛ける
        const midAlpha = 0.038 * (0.85 + Math.sin(t * 1.5) * 0.15) * curtainRibs;
        ...
        // グラデーションの色の配色：
        // 発光ピークのコントラストをなだらかにし、透き通る暗い霧のような緑のみで構築
        const colorTop = "rgba(0, 0, 0, 0)"; // 上端：完全な透明
        const colorUpperDark = "rgba(0, 15, 8, " + (scaleAlpha * 0.005) + ")";    // 上部3分の1付近：極めて薄く暗い深緑
        const colorTealGreen = "rgba(0, 65, 40, " + (scaleAlpha * 0.10) + ")";     // 中間移行部：やや暗めの深緑
        const colorEmerald = "rgba(0, 130, 60, " + (scaleAlpha * 0.24) + ")";      // 中間発光ベース：おだやかなエメラルドグリーン
        const colorPeakGreen = "rgba(0, 150, 70, " + (scaleAlpha * 0.30) + ")";     // 下部8分の1の発光部：主張を抑えたエメラルド（白を完全廃止し、輝度も抑制）
        const colorBottomGreen = "rgba(0, 90, 40, " + (scaleAlpha * 0.15) + ")";    // 下端手前：緩やかに薄くなる緑
        const colorEnd = "rgba(0, 5, 2, 0)"; // 最下端：なだらかに消える
        ...
        grad.addColorStop(0.93, colorPeakGreen);   // 発光の最大ピーク（輝度とアルファを抑え、エメラルドグリーンとして自然に馴染ませる）
