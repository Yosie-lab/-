...
const newBlock = `function drawRealAuroraCurtain() {
    ...
    // 強烈に光り輝くグロー効果（ぼかし）の最大化 (24から45へ大幅引き上げ)
    // 光彩を広く美しく拡散させます
    showerCtx.shadowColor = 'rgba(0, 245, 125, 0.40)';
    showerCtx.shadowBlur = 45;
    ...
    for (let x = 0; x < showerCanvas.width; x += step) {
        ...
        // 縦方向の長さ（高さをさらに長くし、ボカシと合わせて幅広くダイナミックに見せる）
        const curtainHeight = (520 + Math.sin(x * 0.003 - t) * 100) * z;
        ...
        // グラデーションの色の配色：
        // 比率をさらに広げ、なだらかで幅の広い光の帯を構成
        const colorTop = "rgba(0, 0, 0, 0)"; // 上端：完全な透明
        const colorUpperGreen = "rgba(0, 30, 20, " + (scaleAlpha * 0.01) + ")";   // 上部：ほぼ黒に近い暗闇
        const colorTeal = "rgba(0, 120, 100, " + (scaleAlpha * 0.28) + ")";       // 中間上部：シアン・ティール系
        const colorEmerald = "rgba(0, 210, 110, " + (scaleAlpha * 0.70) + ")";     // 中間：メインのエメラルドグリーン
        const colorLightGreen = "rgba(60, 235, 150, " + (scaleAlpha * 0.80) + ")"; // 境界緩和用中間色：明るいミントエメラルド
        const colorPeakWhite = "rgba(210, 255, 235, " + (scaleAlpha * 0.90) + ")";  // 輝度最大部：白寄りの明るいミント（不透明度を抑えて馴染ませる）
        const colorBottomGreen = "rgba(0, 180, 90, " + (scaleAlpha * 0.50) + ")";  // 下端手前：緩やかに薄くなる緑
        const colorEnd = "rgba(0, 10, 5, 0)"; // 最下端：なだらかに消える

        grad.addColorStop(0.0, colorTop);
        grad.addColorStop(0.40, colorUpperGreen);  // 暗い領域を広く維持
        grad.addColorStop(0.65, colorTeal);        // ティールでなめらかに開始
        grad.addColorStop(0.78, colorEmerald);     // エメラルドグリーン
        grad.addColorStop(0.85, colorLightGreen);  // 白へのなだらかなクッション
        grad.addColorStop(0.90, colorPeakWhite);   // 0.90のピーク部分（白）
        grad.addColorStop(0.93, colorLightGreen);  // 緑への戻り用クッション
        grad.addColorStop(0.96, colorBottomGreen); // 下端付近でなだらかに減衰
        grad.addColorStop(1.0, colorEnd);          // 最下端で完全消滅
        
        showerCtx.strokeStyle = grad;
        // 線の太さを大幅に太くし、個々の細い線から「幅広のどっしりとした光の帯」にします
        showerCtx.lineWidth = (step + 5.5) * z;
