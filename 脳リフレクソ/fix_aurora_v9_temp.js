...
const newBlock = `function drawRealAuroraCurtain() {
    ...
    for (let x = 0; x < showerCanvas.width; x += step) {
        ...
        // グラデーションの色の配色：
        // 上部3分の1（0〜33%）を暗い領域とし、残る3分の2（33%〜100%）に発光グラデーションを広げる構成
        const colorTop = "rgba(0, 0, 0, 0)"; // 上端：完全な透明
        const colorUpperDark = "rgba(0, 30, 20, " + (scaleAlpha * 0.03) + ")";   // 上部3分の1付近：暗い深緑（33%まで）
        const colorTeal = "rgba(0, 110, 95, " + (scaleAlpha * 0.35) + ")";        // 立ち上がり：シアン・ティール系
        const colorEmerald = "rgba(0, 220, 115, " + (scaleAlpha * 0.78) + ")";     // エメラルドグリーン（3分の2の大部分をカバー）
        const colorLightGreen = "rgba(70, 240, 155, " + (scaleAlpha * 0.85) + ")"; // 境界緩和用中間色
        const colorPeakWhite = "rgba(215, 255, 238, " + (scaleAlpha * 0.98) + ")";  // 発光ピーク：白寄りの明るいミント
        const colorBottomGreen = "rgba(0, 150, 75, " + (scaleAlpha * 0.40) + ")";  // 下端手前：緩やかに薄くなる緑
        const colorEnd = "rgba(0, 10, 5, 0)"; // 最下端：なだらかに消える

        grad.addColorStop(0.0, colorTop);
        grad.addColorStop(0.33, colorUpperDark);   // 上部3分の1（ここまでを暗い部分に制限）
        grad.addColorStop(0.55, colorTeal);        // 0.55からなめらかに立ち上がり
        grad.addColorStop(0.75, colorEmerald);     // 美しいエメラルドグリーンを幅広く表現
        grad.addColorStop(0.84, colorLightGreen);  // 白へのなだらかなクッション
        grad.addColorStop(0.90, colorPeakWhite);   // 発光ピーク
