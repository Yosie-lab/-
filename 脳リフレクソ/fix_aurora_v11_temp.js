...
const newBlock = `function drawRealAuroraCurtain() {
    ...
    // 強烈に光り輝くグロー効果（ぼかし）の極大化 (80から95へ引き上げ)
    // 霧の部分も含めて光彩を極めて広く美しく拡散させ、幻想的なイメージを極限まで高めます
    showerCtx.shadowColor = 'rgba(0, 250, 120, 0.35)';
    ...
    for (let x = 0; x < showerCanvas.width; x += step) {
        ...
        // グラデーションの色の配色：
        // 全体の色をエメラルドグリーン主体の緑にし、上部3分の1（0〜33%）が暗い霧、下部8分の1（87.5%〜100%）が強い発光
        const colorTop = "rgba(0, 0, 0, 0)"; // 上端：完全な透明
        const colorUpperDark = "rgba(0, 20, 10, " + (scaleAlpha * 0.01) + ")";     // 上部3分の1付近：極めて薄く暗い深緑
        const colorTealGreen = "rgba(0, 90, 60, " + (scaleAlpha * 0.22) + ")";     // 中間移行部：やや暗めの深緑（ティールグリーン）
        const colorEmerald = "rgba(0, 180, 80, " + (scaleAlpha * 0.55) + ")";      // 中間発光ベース：発光量を抑えたエメラルドグリーン
        const colorPeakGreen = "rgba(0, 220, 95, " + (scaleAlpha * 0.72) + ")";     // 下部8分の1：強いエメラルドグリーン（発光開始）
        const colorPeakWhite = "rgba(200, 255, 225, " + (scaleAlpha * 0.82) + ")";   // 輝度最大部：緑と自然に溶け合うミント（発光量をグッと抑制）
        const colorBottomGreen = "rgba(0, 130, 60, " + (scaleAlpha * 0.30) + ")";   // 下端手前：緩やかに薄くなる緑
        ...
        grad.addColorStop(0.875, colorEmerald);    // ここから下部8分の1の強い発光が始まる
        grad.addColorStop(0.91, colorPeakGreen);   // 強いエメラルド
        grad.addColorStop(0.93, colorPeakWhite);   // 発光ピーク（白を抑え、エメラルドに完全に馴染ませる）
        grad.addColorStop(0.95, colorPeakGreen);   // 強いエメラルド
