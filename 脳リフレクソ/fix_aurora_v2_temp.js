...
const newBlock = `function drawRealAuroraCurtain() {
    ...
    // 描画される線全体にエメラルドグリーンの光彩をまとわせ、色の境界を滑らかに溶かします
    showerCtx.shadowColor = 'rgba(0, 245, 120, 0.45)';
    ...
    for (let x = 0; x < showerCanvas.width; x += step) {
        ...
        // コントラストを前回の 0.55〜1.0 から 0.70〜1.0 に和らげ、なめらかさを向上
        ...
        // グラデーションの色の配色：
        // 白と緑がはっきり分かれず、上部は暗闇に溶け込み、下部は強烈に光る構成
        const colorTop = "rgba(0, 0, 0, 0)"; // 上端：完全な透明
        const colorUpperGreen = "rgba(0, 30, 20, " + (scaleAlpha * 0.02) + ")"; // 上部〜中間上部：ほぼ黒に近い極めて淡く暗い深緑
        const colorMidGreen = "rgba(0, 160, 95, " + (scaleAlpha * 0.40) + ")";  // 中間部：徐々に立ち上がるエメラルド
        const colorPeakGreen = "rgba(0, 240, 125, " + (scaleAlpha * 0.90) + ")"; // 発光部：鮮やかで明るいエメラルドグリーン
        const colorMintWhite = "rgba(225, 255, 242, " + (scaleAlpha * 1.30) + ")"; // 輝度最大部：エメラルドと溶け合うミントホワイト（最大光）
        ...
        grad.addColorStop(0.50, colorUpperGreen);  // 0.50まで暗い領域を長く維持（上部は暗闇に紛れる）
        grad.addColorStop(0.74, colorMidGreen);    // 0.74から徐々に立ち上がる
        grad.addColorStop(0.87, colorPeakGreen);   // 0.87で鮮やかなエメラルドグリーン
        grad.addColorStop(0.92, colorMintWhite);   // 0.92付近にミントホワイトを配置（エメラルドと自然に溶け合います）
        grad.addColorStop(0.96, colorBottomGreen); // 下端付近で減衰
